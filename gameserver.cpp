...
namespace game
{
    namespace gameserver
    {
        ...
        int GameServer::onGameSoMessage(const long lPlayerID, const vector<char> &vecMsgData)
        {
            ...
            //校验版本
            if (somsg.nver() != XGameDZProtoClub::iProtoVersion)
            {
                LOG_WARN("client proto version : \"" << somsg.nver() << "\" server proto version : \"" << XGameDZProtoClub::iProtoVersion << "\";");
            }

            //校验命令字
            if (somsg.ncmd() > XGameDZProtoClub::MSG_ID_END || somsg.ncmd() < XGameDZProtoClub::MSG_ID_BEGIN)
            {
        ...
        int GameServer::doSendGameData(const long &pid, int eMSG, const vector<char> &vecData)
        {
            ...
            sTSoMsg.set_nver(XGameDZProtoClub::iProtoVersion);
